...
RSpec.describe 'Task', type: :system do
  ...
  describe 'Task編集' do
    let!(:task) { create(:task) }
    let!(:status_done_task) { create(:task, :done_task) }
    ...
    context '正常系' do
      ...
      it '既にステータスが完了のタスクのステータスを変更した場合、Taskの完了日が更新されないこと' do
        ...
        visit edit_project_task_path(project, status_done_task)
        select 'todo', from: 'Status'
        click_button 'Update Task'
        expect(page).to have_content('todo')
        expect(page).not_to have_content(Time.current.strftime('%Y-%m-%d'))
        expect(current_path).to eq project_task_path(project, status_done_task)
      end
    end
  end
